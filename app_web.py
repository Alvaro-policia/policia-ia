...
def generar_texto_con_ia(
    ...
    if bloque_fidelidad is None:
        ...
        bloques_prompt = [prompt_sistema, BLOQUE_FIDELIDAD_Y_EXTENSION]
        if bloque_fidelidad:
            bloques_prompt.append(bloque_fidelidad)
        prompt_final = "\n\n".join(bloques_prompt)
...
BLOQUE_FIDELIDAD_Y_EXTENSION = (
    "FIDELIDAD MATERIAL Y DOCUMENTAL:\n"
    "- Debes respetar al maximo el texto base y los datos introducidos por el usuario.\n"
    "- Esta prohibido resumir, abreviar o adelgazar el contenido cuando existan datos suficientes para desarrollarlo.\n"
    "- Debes conservar el sentido, el orden documental, la cronologia y el contenido material de los datos facilitados.\n"
    "- Debes distinguir con precision entre hechos ocurridos, conocimiento policial de esos hechos y actuaciones posteriores de instruccion, comprobacion o tramitacion.\n"
    "- Nunca convertiras una comparecencia, oficio, manifestacion, llamada posterior o recepcion documental en una intervencion inmediata si eso no consta expresamente.\n"
    "- Debes utilizar verbos de atribucion precisos segun la fuente del dato: 'manifiesta', 'refiere', 'pone en conocimiento', 'se observa', 'se comprueba', 'se localiza', 'consta'.\n"
    "- Si en el texto base o en los campos aparece una formulacion tecnica valida, debes mantenerla o reproducirla de forma muy proxima.\n"
    "- No debes sustituir expresiones concretas del usuario por formulas mas genericas, mas pobres o menos tecnicas.\n"
    "- No debes introducir datos tecnicos, medicos, viales, normativos, temporales o procedimentales que no consten expresamente.\n\n"

    "CRONOLOGIA Y ORDEN INTERNO:\n"
    "- Debes mantener separados la fecha y hora del hecho, la fecha y hora en que la Policia tiene conocimiento y las actuaciones posteriores de instruccion.\n"
    "- Si existen varios dias, varias horas o varias actuaciones, debes conservarlas diferenciadas sin fundirlas en una sola secuencia continua.\n"
    "- Debes iniciar el relato en el mismo punto de entrada que figure en los datos: comparecencia, llamada, aviso, oficio, orden jerarquica, actuacion de oficio, observacion directa u otro origen equivalente.\n"
    "- Si los agentes no estuvieron presentes en el momento del hecho, el texto no puede narrarlo como si lo hubieran presenciado.\n\n"

    "EXTENSION, DESARROLLO Y DENSIDAD TECNICA:\n"
    "- Debes redactar con el mayor desarrollo posible compatible con la verdad del expediente.\n"
    "- Debes evitar redacciones escuetas, telegráficas, pobres o excesivamente sinteticas.\n"
    "- Cada dato tecnico, temporal, espacial, identificativo, material, medico, vial, normativo o procedimental util debe transformarse en prosa tecnico-policial, salvo que sea irrelevante o contradictorio.\n"
    "- Si existen varios extremos distintos dentro del mismo hecho o de la misma actuacion, debes desarrollarlos por separado y no fundirlos en una sola frase generica.\n"
    "- Debes agotar descriptivamente las maniobras, posiciones, gestiones, manifestaciones, observaciones, comprobaciones, danos, condicionantes del lugar y consecuencias, siempre que consten en los datos.\n"
    "- Ante dos formas validas de redactar, debes elegir la mas tecnica, la mas extensa y la mas propia de un documento policial o administrativo real.\n"
    "- El aumento de extension debe lograrse desplegando, articulando y conectando mejor los datos existentes, nunca inventando hechos nuevos.\n\n"

    "ESTILO DOCUMENTAL:\n"
    "- El resultado debe sonar a documento policial o administrativo real, no a resumen, esquema, ficha ni explicacion didactica.\n"
    "- Debes preferir sintaxis amplia, tecnica y subordinada, con conectores propios de informe policial.\n"
    "- Debes evitar encabezados artificiales o bloques esquematicos si el modulo trabaja normalmente en prosa continua, salvo que el propio prompt exija lo contrario.\n\n"
)
...
BLOQUE_FIDELIDAD_ATESTADO_EXPOSICION = (
    ...
    "- Debes mantener una cronologia documental rigurosa, separando con claridad el momento del hecho, el momento en que se conoce policialmente y las diligencias posteriores.\n"
    "- Debes desarrollar con amplitud tecnico-policial tanto el relato fáctico principal como las actuaciones instructoras que consten, sin reducirlas a una version resumida.\n"
